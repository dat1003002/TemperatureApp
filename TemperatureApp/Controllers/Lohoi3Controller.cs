...
{
    public class Lohoi3Controller : Controller
    {
        private readonly ILogger<Lohoi3Controller> _logger;
        private readonly DatabaseService _databaseService;
        private readonly ApplicationDbContext _context;

        public Lohoi3Controller(ILogger<Lohoi3Controller> logger, DatabaseService databaseService, ApplicationDbContext context)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> IndexAsync(DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                var query = _context.BTLH3.AsQueryable();
            ...
            {
                _logger.LogError($"Lỗi khi xử lý Lohoi3: {ex.Message}");
        ...
        public async Task<IActionResult> ExportToExcel(DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                {
                    var worksheet = package.Workbook.Worksheets.Add("Dữ liệu Lò Hơi 3");
                    ...
                    for (int i = 0; i < data.Count; i++)
                    {
                        ...
                        worksheet.Cells[i + 2, 2].Value = item.APTC / 10.0;
                    ...
                    string fileName = $"LoHoi3_{DateTime.Now:yyyyMMddHHmmss}.xlsx";
